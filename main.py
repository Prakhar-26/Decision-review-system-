...
def pending(decision):
    ...
    frame = cv2.cvtColor(cv2.imread("pending1.png"), cv2.COLOR_BGR2RGB)
    frame = imutils.resize(frame, width=SET_WIDTH, height=SET_HEIGHT)
    frame = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
    canvas.image = frame
    canvas.create_image(0,0, image=frame, anchor=tkinter.NW)
    ...
    if decision == 'out':
        decisionImg = "out1.png"
    else:
        decisionImg = "not_out1.png"
...
cv_img = cv2.cvtColor(cv2.imread("lords.jpg"), cv2.COLOR_BGR2RGB)
